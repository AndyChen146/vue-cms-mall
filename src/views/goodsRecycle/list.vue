<template>
  <div class="con-right">
          ...
          <div v-show="!nodatashow">
            <div class="td" v-for="(item,index) in list" :key="index">
              ...
              <span class="span60">
                ...
                <p v-else>未通过</p>
                <a href="javascript:;" class="C0067ED" @click="openAuditDetail(item.product_id)">审核详情</a>
              </span>
              <span class="span80">
                <p>
                  <a class="edit C0067ED" @click="reduction(item.product_id)">还原</a>
                  <a href="javascript:;" class="audit delete" @click="del(item.product_id)">删除</a>
                </p>
              </span>
            </div>
    ...
    <el-dialog class="auditDetail" title="审核详情" :visible.sync="dialogVisible" width="608" :before-close="handleClose">
      <div class="box">
        <table>
          <tr>
            <th>审核时间</th>
...
export default {
  ...
  methods: {
    ...
    ...mapActions('lyf', ['getProductList', 'getProductCounts', 'getCategoryList', 'getBrandList', 'getProductAuditDetail', 'deleteProductId', 'productReduction']),
    //获取商品列表
    getProductLists () {
      var _self = this;
      _self.loading = true
    ...
    del (id) {
      var _self = this;
      _self.$confirm('此操作将永久删除该商品，是否继续？', '提示', {
    ...
    reduction (id) {
      var _self = this;
      _self.productReduction({
        product_id: id,
      }).then(res => {
        if (res.status == "0") {
          this.$message({
            type: 'success',
            message: '还原成功，可在商品列表查看',
            customClass: 'toast'
          });
          _self.getProductLists()
          _self.getCountList()
